...
import generators
import sql_stmt
from . import administrators_bp
import decorators
import administrators_backend
import payments


@administrators_bp.route('/dashboard', methods=['GET'])
@decorators.admin_login_checker
@decorators.handle_errors
def dashboard():
    cur_user = generators.cur_user_details(session['maarifa-education-id'])
    return render_template('administrators/user-dashboard.html', admin_name=cur_user[0], )


@administrators_bp.route('/add-lower-learning', methods=['GET', 'POST'])
@decorators.admin_login_checker
@decorators.handle_errors
def add_lower_learning():
    if request.method == 'POST':
        cur_user = generators.cur_user_details(session['maarifa_education_id'])
        result_set = administrators_backend.add_lower_learning(cur_user[2], )
        if result_set:
            session['create-flag'] = True
            return redirect(url_for('administrators.view_lower_learning'))
        return redirect(url_for('administrators.add_lower_learning'))
    else:
        return render_template('add-lower-learning.html', admin_name=session['admin-name'], admin_pic=session['admin-pic'])


@administrators_bp.route('/lower-learning-institutions', methods=['GET'])
@decorators.admin_login_checker
@decorators.handle_errors
def lower_learning_institutions():
    create_flag = False
    if 'create-flag' in session:
        create_flag = session['create-flag']
        session.pop('create-flag')
    return render_template('lower-learning-institution.html', create_flag=create_flag)
...
def user_profile():
    ...
    if request.method == 'POST':
        ...
        return render_template('administrators/profile.html', admin_name=cur_user[0], admin_details=result_set, update_flag=update_flag)
...
@administrators_bp.route('/purchase-sms', methods=['POST'])
@decorators.admin_login_checker
@decorators.handle_errors
def user_purchase_sms():
    ...
    if sms_purchase:
        ...
        # update the administrators user sms type.
